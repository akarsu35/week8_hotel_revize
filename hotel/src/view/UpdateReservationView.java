...
public class UpdateReservationView extends Layout{
    ...
    public UpdateReservationView(Room room, String start_date, String finish_date, int adult_num, int child_num,Reservation reservation) {//filtrelemeden gelen giriş çıkış terihi ,ytşkn sayısı çck sayısı
        this.add(container);
        this.room=room;
        ...
        this.fld_price.setText(String.valueOf(this.reservationManager.getById(reservation.getId()).getTotal_price()));
        this.fld_quest_count.setText(String.valueOf(this.reservationManager.getById(reservation.getId()).getQuest_count()));
        ...
        if(reservation!=null){
            this.fld_quest_name.setText(reservation.getQuest_name());
            this.fld_mail.setText(reservation.getQuest_mail());
            this.fld_quest_id.setText(reservation.getQuest_identy());
            this.fld_phone.setText(reservation.getQuest_phone());
        }
        btn_save.addActionListener(e -> {

            JTextField[] checkFieldList = {this.fld_quest_name,this.fld_quest_id,this.fld_mail,this.fld_phone};
            if (Helper.isFieldListEmpty(checkFieldList)) {
                Helper.showMsg("fill");
            } else {
                boolean result;

                // Seçili rezervasyonu kontrol et
                ...
                //System.out.println(this.reservation.getQuest_identy()+" : "+this.reservation.getQuest_name()+" : "+this.reservation.getQuest_mail()+" : "+this.reservation.getQuest_phone());


                    // Güncelleme işlemini gerçekleştir
                if(this.reservation.getId()!=0){
                    result = this.reservationManager.updateReservation(this.reservation);
                }else{
                    result = false;
                }
                if (result) {
